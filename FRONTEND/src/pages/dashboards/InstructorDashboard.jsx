      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-slate-500 mt-1">Assigned to you</div>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-bold text-slate-800">{stats.avgScore ? `${stats.avgScore}%` : '-'}</div>
            <div className="text-xs text-slate-500 mt-1">Cohort average</div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-slate-800">Student Progress</h3>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col items-center text-center">
            <div className="w-20 h-20 rounded-2xl bg-blue-600 text-white flex items-center justify-center text-2xl font-bold shadow-lg shadow-blue-500/30 mb-4 overflow-hidden">
            ...
            {sessions.length > 0 ? (
              <div className="space-y-3">
                {sessions.map((session, idx) => (
                  <div key={idx} className="p-3 border border-slate-100 rounded-xl hover:border-blue-100 transition-colors">
                    <div className="flex justify-between items-start mb-1">
                      <span className="font-semibold text-sm text-slate-800">{session.title || session.studentName}</span>
                      <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase tracking-wide">
                        {session.type || 'Review'}
                      </span>
                    </div>
                    <div className="text-xs text-slate-500">{session.time || 'Time TBD'} &bull; {session.date || 'Today'}</div>
                  </div>
                ))}
              </div>
            ) : (
