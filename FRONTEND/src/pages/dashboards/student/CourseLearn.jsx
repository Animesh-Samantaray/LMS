      ...
      <div className="max-w-[1400px] mx-auto px-6 grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-[var(--lms-surface)] rounded-3xl p-6 flex flex-col md:flex-row gap-8 shadow-sm border border-[var(--lms-border)]">
            ...
            <div className="flex-1 flex flex-col justify-center">
              ...
              <h1 className="text-3xl font-extrabold text-[var(--lms-text-primary)] mb-2 leading-tight">
                {course.title}
              </h1>
              <p className="text-sm text-[var(--lms-text-secondary)] mb-5 leading-relaxed line-clamp-2">
          ...
          {activeTab === 'curriculum' && (
            <div className="bg-[var(--lms-surface)] rounded-3xl shadow-sm border border-[var(--lms-border)] p-8">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-2xl font-extrabold text-[var(--lms-text-primary)] mb-1">Course Curriculum</h2>
              ...
              <div className="space-y-4">
                {units.map((unit, unitIdx) => {
                  ...
                  return (
                    <div key={unit._id} className="border border-[var(--lms-border)] rounded-2xl overflow-hidden transition-all duration-300">
                      <div 
                        onClick={() => toggleUnit(unit._id)}
                        className="flex items-center justify-between p-5 bg-[var(--lms-surface)] hover:bg-[var(--lms-surface-subtle)] cursor-pointer select-none"
                      >
                        <div className="flex items-center gap-5">
                          <div className="w-10 h-10 rounded-full bg-[var(--lms-surface-elevated)] text-[var(--lms-text-secondary)] font-extrabold text-base flex items-center justify-center shrink-0">
                            {unitIdx + 1}
                          </div>
                          <div>
                            <h3 className="font-extrabold text-[var(--lms-text-primary)] text-base">{unit.title}</h3>
                            <p className="text-xs text-[var(--lms-text-muted)] font-medium mt-0.5">{unit.description || "Course unit section"}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-6">
                          <div className="text-right hidden sm:block">
                            <p className="text-xs font-bold text-[var(--lms-text-secondary)]">{unitTotalLessons} Lessons • {unitDurStr}</p>
                            <div className="flex items-center gap-2 mt-1.5 justify-end">
                              <div className="w-24 h-1.5 bg-[var(--lms-border)] rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500 rounded-full transition-all duration-500" style={{ width: `${unitProgressPct}%` }}></div>
                              </div>
                              <span className="text-[10px] font-bold text-[var(--lms-text-secondary)] w-6 text-right">{unitProgressPct}%</span>
                            </div>
                          </div>
                          <ChevronDown size={20} className={`text-[var(--lms-text-muted)]/70 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />
                        </div>
                      </div>

                      {isExpanded && (
                        <div className="bg-[var(--lms-surface-subtle)] border-t border-[var(--lms-border)]/50 p-2 sm:p-4 space-y-1">
                          {unitLessons.map((lesson, lessonIdx) => {
                            ...
                            return (
                              <div 
                                key={lesson._id}
                                className={`flex items-center justify-between p-3 rounded-xl transition-all group border border-transparent ${isLocked ? 'opacity-60' : 'hover:bg-[var(--lms-surface)] hover:shadow-sm hover:border-[var(--lms-border)]'}`}
                              >
                                <div className="flex items-center gap-4">
                                  <div className={`${isLocked ? 'text-[var(--lms-text-muted)]/70' : 'text-rose-500'}`}>
                                    {lesson.contentType === 'Video' ? <PlayCircle size={18} /> : <FileText size={18} />}
                                  </div>
                                  <span className="text-xs font-bold text-[var(--lms-text-muted)]/70 w-6">{unitIdx + 1}.{lessonIdx + 1}</span>
                                  <span className={`text-sm font-semibold transition-colors ${isLocked ? 'text-[var(--lms-text-muted)]' : 'text-[var(--lms-text-secondary)] group-hover:text-blue-600'}`}>
                                    {lesson.title}
                                  </span>
                                </div>
                                <div className="flex items-center gap-6">
                                  <div className="flex items-center gap-1.5 text-xs font-semibold text-[var(--lms-text-muted)]">
                                    <Clock size={14} />
                                    <span>{lesson.duration || 0}:00</span>
                                  </div>
                                  <div className="flex items-center gap-1.5 text-xs font-semibold text-[var(--lms-text-muted)] w-24">
                                    <FileText size={14} />
                                  ...
                                  <button
                                    ...
                                    disabled={isLocked}
                                    className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors w-28 text-center ${isLocked ? 'bg-[var(--lms-surface-elevated)] text-[var(--lms-text-muted)]/70 cursor-not-allowed' : 'bg-blue-50 hover:bg-blue-100 text-blue-600'}`}
                                  >
                                    {isLocked ? "Locked" : isCompleted ? "Watch Again" : lesson.contentType === 'Video' ? "Watch" : "Read"}
                                  </button>

                                  {isLocked ? (
                                    <div className="p-1"><Circle size={20} className="text-[var(--lms-border)]" /></div>
                                  ) : (
                                    <button 
                                      onClick={(e) => handleCompleteLesson(lesson._id, e)}
                                      disabled={isCompleted}
                                      className={`p-1 rounded-full transition-colors ${isCompleted ? 'cursor-default' : 'hover:bg-[var(--lms-border)]'}`}
                                      title={isCompleted ? "Completed" : "Mark as completed"}
                                    >
                                      {isCompleted ? (
                                        <CheckCircle2 size={20} className="text-emerald-500 fill-emerald-50" />
                                      ) : (
                                        <Circle size={20} className="text-[var(--lms-text-muted)]/40" />
                                      )}
                                    </button>
                                  )}
                                </div>
          ...
          {activeTab === 'overview' && (
            <div className="bg-[var(--lms-surface)] rounded-3xl shadow-sm border border-[var(--lms-border)] p-8">
              <h2 className="text-2xl font-extrabold text-[var(--lms-text-primary)] mb-6">Course Overview</h2>
              <div className="prose max-w-none text-[var(--lms-text-secondary)] leading-relaxed space-y-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-[var(--lms-text-primary)] mb-3">What you'll learn</h3>
          ...
          {activeTab === 'resources' && (
            <div className="bg-[var(--lms-surface)] rounded-3xl shadow-sm border border-[var(--lms-border)] p-8">
              <h2 className="text-2xl font-extrabold text-[var(--lms-text-primary)] mb-6">All Course Resources</h2>
              {allResources.length === 0 ? (
                <div className="text-center py-12">
                  <Download size={48} className="text-[var(--lms-border)] mx-auto mb-4" />
                  <h3 className="text-lg font-bold text-[var(--lms-text-secondary)] mb-1">No resources yet</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {allResources.map((res, i) => (
                    <div 
                      ...
                      className="flex items-center gap-4 p-4 border border-[var(--lms-border)] rounded-2xl hover:border-blue-300 hover:shadow-md transition-all cursor-pointer group"
                    >
                      <div className="w-12 h-12 rounded-xl bg-blue-50 text-blue-500 flex items-center justify-center shrink-0">
                        <FileText size={24} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-base font-bold text-[var(--lms-text-primary)] group-hover:text-blue-600 transition-colors truncate">{res.title}</h4>
                        <p className="text-xs font-bold text-[var(--lms-text-muted)] uppercase mt-0.5">{res.resourceType}</p>
                      </div>
                      <button className="w-10 h-10 rounded-full border border-[var(--lms-border)] flex items-center justify-center text-blue-600 hover:bg-blue-50 hover:border-blue-200 transition-colors">
                        <Download size={18} />
                      </button>
                    </div>
                  ))}
          ...
          {activeTab === 'qa' && (
            <div className="bg-[var(--lms-surface)] rounded-3xl shadow-sm border border-[var(--lms-border)] p-8">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-extrabold text-[var(--lms-text-primary)]">Questions & Answers</h2>
                <button 
                  onClick={() => setShowQuestionModal(true)}
                  className="px-5 py-2.5 bg-[var(--lms-text-primary)] hover:opacity-90 text-[var(--lms-surface)] font-bold rounded-xl text-sm transition-colors shadow-lg shadow-[var(--lms-text-primary)]/10"
                >
                  Ask a Question
                </button>
              </div>
              ...
              <div className="text-center py-12">
                <MessageSquare size={48} className="text-[var(--lms-border)] mx-auto mb-4" />
                {questionSubmitted ? (
                  <>
                    <h3 className="text-lg font-bold text-[var(--lms-text-secondary)] mb-1">Question Submitted!</h3>
                    <p className="text-[var(--lms-text-muted)] text-sm">Your question has been sent to the instructor. They will respond shortly.</p>
                  </>
                ) : (
                  <>
                    <h3 className="text-lg font-bold text-[var(--lms-text-secondary)] mb-1">No questions yet</h3>
                    <p className="text-[var(--lms-text-muted)] text-sm">Be the first to ask a question in this course!</p>
                  </>
                )}
              </div>
            </div>
          ...
          {activeTab === 'instructor' && (
            <div className="bg-[var(--lms-surface)] rounded-3xl shadow-sm border border-[var(--lms-border)] p-8">
              <h2 className="text-2xl font-extrabold text-[var(--lms-text-primary)] mb-8">About the Instructor</h2>
              
              <div className="flex flex-col md:flex-row gap-8">
                <div className="w-32 h-32 rounded-full overflow-hidden shrink-0 border-4 border-[var(--lms-surface-subtle)] bg-[var(--lms-surface-elevated)]">
                  {course.createdBy?.profileImage ? (
                    <img src={course.createdBy.profileImage} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-4xl font-black text-[var(--lms-text-muted)]/40 uppercase">
                ...
                <div>
                  <h3 className="text-xl font-bold text-[var(--lms-text-primary)] mb-1">{course.createdBy?.name || "Instructor"}</h3>
                  <p className="text-sm font-bold text-blue-600 mb-4">{course.category?.name || course.category || "Senior Developer & Educator"}</p>
                  
                  <div className="flex flex-wrap gap-4 mb-6">
                    <div className="flex items-center gap-1.5 text-sm font-semibold text-[var(--lms-text-secondary)]">
                      <Star size={16} className="text-amber-500 fill-current" />
                      4.8 Instructor Rating
                    </div>
                    <div className="flex items-center gap-1.5 text-sm font-semibold text-[var(--lms-text-secondary)]">
                      <User size={16} className="text-[var(--lms-text-muted)]/70" />
                      10,000+ Students
                    </div>
                    <div className="flex items-center gap-1.5 text-sm font-semibold text-[var(--lms-text-secondary)]">
                      <PlayCircle size={16} className="text-[var(--lms-text-muted)]/70" />
                      5 Courses
                    </div>
                  </div>
                  
                  <div className="prose text-[var(--lms-text-secondary)] text-sm leading-relaxed">
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-6 mb-8">
              <div className="relative w-24 h-24 shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-xl font-black text-[var(--lms-text-primary)] leading-none">{completionPercentage}%</span>
                  <span className="text-[9px] font-bold text-[var(--lms-text-muted)] uppercase mt-0.5">Completed</span>
                </div>
              </div>

              <div className="space-y-3">
                <p className="text-sm font-bold text-[var(--lms-text-secondary)]">{completedCount} of {totalLessons} lessons completed</p>
                <div className="flex items-center gap-2 text-xs font-semibold text-[var(--lms-text-muted)]">
                  <Clock size={14} />
          ...
          {upNextLessons.length > 0 && (
            <div className="bg-[var(--lms-surface)] rounded-3xl shadow-sm border border-[var(--lms-border)] p-6 flex flex-col">
              <h3 className="text-sm font-bold text-[var(--lms-text-muted)] mb-4">Up next</h3>
              <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-2 flex-1">
                {upNextLessons.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-20 h-14 bg-gray-900 rounded-lg overflow-hidden relative shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-bold text-[var(--lms-text-muted)] mb-0.5 truncate">{item.unit.title || "Unit"}</p>
                        <h4 className="text-sm font-bold text-[var(--lms-text-primary)] truncate group-hover:text-blue-600 transition-colors">{item.lesson.title}</h4>
                        <div className="flex items-center gap-1 text-[10px] font-bold text-[var(--lms-text-muted)]/70 mt-1">
                          <Clock size={10} /> {item.lesson.duration || 0}:00
                        </div>
                      </div>
                      <ChevronRight size={16} className="text-[var(--lms-text-muted)]/40 shrink-0" />
                    </div>
                  );
                })}
              </div>
          ...
          <div className="bg-[var(--lms-surface)] rounded-3xl shadow-sm border border-[var(--lms-border)] p-6 flex flex-col">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-extrabold text-[var(--lms-text-primary)]">Course Resources</h3>
              <span className="text-xs font-bold text-[var(--lms-text-muted)]">{allResources.length} files</span>
            </div>
            
            <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-2 flex-1">
              {allResources.length === 0 ? (
                <p className="text-sm text-[var(--lms-text-muted)] text-center py-4">No additional resources available.</p>
              ) : (
                allResources.map((res, i) => (
                  <div 
                    key={res._id || i}
                    onClick={(e) => handleResourceClick(res.fileUrl || res.externalUrl, e)}
                    className="flex items-center gap-4 p-2 rounded-xl hover:bg-[var(--lms-surface-subtle)] transition-colors group cursor-pointer"
                  >
                    <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-500 flex items-center justify-center shrink-0">
                      <FileText size={18} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-sm font-bold text-[var(--lms-text-primary)] group-hover:text-blue-600 transition-colors truncate">{res.title}</h4>
                      <p className="text-[10px] font-bold text-[var(--lms-text-muted)] uppercase">{res.resourceType}</p>
                    </div>
                    <button className="w-8 h-8 rounded-full border border-[var(--lms-border)] flex items-center justify-center text-blue-600 hover:bg-blue-50 hover:border-blue-200 transition-colors">
                      <Download size={14} />
                    </button>
                  </div>
                ))
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-bold text-[var(--lms-text-secondary)]">{completionPercentage}%</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[var(--lms-surface)] rounded-3xl w-full max-w-lg p-6 sm:p-8 shadow-2xl">
            ...
            <textarea
              ...
              className="w-full h-32 px-4 py-3 rounded-2xl bg-[var(--lms-surface-subtle)] border border-[var(--lms-border)] text-sm font-medium text-[var(--lms-text-primary)] focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 resize-none transition-all mb-6"
            ></textarea>
            
            <div className="flex items-center justify-end gap-3">
              <button 
                onClick={() => setShowQuestionModal(false)}
                className="px-5 py-2.5 rounded-xl text-sm font-bold text-[var(--lms-text-secondary)] hover:bg-[var(--lms-surface-elevated)] transition-colors"
              >
                Cancel
              </button>
              <button 
