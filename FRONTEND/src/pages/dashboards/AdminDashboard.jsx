...
const AdminDashboard = () => {
  ...
  return (
    <DashboardLayout sidebarItems={sidebarItems} roleTitle="ADMIN">
      ...
      <div className="bg-white dark:bg-slate-900 pink:bg-pink-50 rounded-2xl p-6 md:p-8 text-white flex flex-col md:flex-row items-start md:items-center justify-between shadow-lg dark:shadow-none shadow-slate-900/20">
        <div>
          ...
          <p className="text-slate-700 dark:text-slate-300 pink:text-pink-800 text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        <div className="bg-white dark:bg-white dark:bg-slate-900 pink:bg-pink-50 p-5 rounded-2xl border border-slate-100 dark:border-slate-800/50 shadow-sm dark:shadow-none flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-bold text-slate-800 dark:text-slate-800 dark:text-slate-200 pink:text-pink-900">{stats.totalUsers ?? (Array.isArray(usersList) ? usersList.length : '-')}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-white dark:bg-slate-900 pink:bg-pink-50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800/50 shadow-sm dark:shadow-none">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-slate-800 dark:text-slate-800 dark:text-slate-200 pink:text-pink-900">Recent Users</h3>
                ...
                {recentUsers.length > 0 ? recentUsers.map((u, idx) => (
                  <tr key={u.id || idx} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                    <td className="py-3 px-2">
                      <div className="font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300 pink:text-pink-800 text-sm">{u.name || 'Unknown'}</div>
                    </td>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-white dark:bg-slate-900 pink:bg-pink-50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800/50 shadow-sm dark:shadow-none flex flex-col items-center text-center">
            <div className="w-20 h-20 rounded-2xl bg-white dark:bg-slate-900 pink:bg-pink-50 text-white flex items-center justify-center text-2xl font-bold shadow-lg dark:shadow-none shadow-slate-900/20 mb-4 overflow-hidden">
            ...
            <div className="w-full grid grid-cols-3 gap-2 border-t border-slate-100 dark:border-slate-800/50 pt-6">
              <div className="text-center">
                <div className="text-lg font-bold text-slate-800 dark:text-slate-800 dark:text-slate-200 pink:text-pink-900">{Array.isArray(usersList) ? usersList.length : '-'}</div>
              ...
              <div className="text-center">
                <div className="text-lg font-bold text-slate-800 dark:text-slate-800 dark:text-slate-200 pink:text-pink-900">{stats.mentors ?? '-'}</div>
          ...
          <div className="bg-white dark:bg-white dark:bg-slate-900 pink:bg-pink-50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800/50 shadow-sm dark:shadow-none">
            <h3 className="font-bold text-slate-800 dark:text-slate-800 dark:text-slate-200 pink:text-pink-900 mb-4">System Health</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600 dark:text-coffee-400">Uptime</span>
                <span className="text-sm font-semibold text-emerald-600">{stats.uptime ? `${stats.uptime}%` : '-'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600 dark:text-coffee-400">API Latency</span>
                <span className="text-sm font-semibold text-blue-600">{stats.latency ? `${stats.latency}ms` : '-'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600 dark:text-coffee-400">Database Load</span>
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300 pink:text-pink-800">{stats.dbLoad ? `${stats.dbLoad}%` : '-'}</span>
              </div>
            </div>
          </div>
        </div>
