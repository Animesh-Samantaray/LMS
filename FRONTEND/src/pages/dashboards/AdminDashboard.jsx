...
const AdminDashboard = () => {
  ...
  return (
    <DashboardLayout sidebarItems={sidebarItems} roleTitle="ADMIN">
      ...
      <div className="bg-slate-900 rounded-2xl p-6 md:p-8 text-white flex flex-col md:flex-row items-start md:items-center justify-between shadow-lg shadow-slate-900/20">
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-bold text-slate-800">{stats.totalUsers ?? (Array.isArray(usersList) ? usersList.length : '-')}</div>
            <div className="text-xs text-slate-500 mt-1">{stats.newUsersThisWeek ? `+${stats.newUsersThisWeek} this week` : 'Active accounts'}</div>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-slate-500 mt-1">{stats.activeCourses ? `${stats.activeCourses} active today` : 'Total published'}</div>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-slate-500 mt-1">Avg course rate</div>
          </div>
          <div className="w-10 h-10 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center">
            <Activity size={20} />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {recentUsers.length > 0 ? recentUsers.map((u, idx) => (
                  <tr key={u.id || idx} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors">
                    <td className="py-3 px-2">
                      <div className="font-semibold text-slate-700 text-sm">{u.name || 'Unknown'}</div>
                    </td>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col items-center text-center">
            <div className="w-20 h-20 rounded-2xl bg-slate-900 text-white flex items-center justify-center text-2xl font-bold shadow-lg shadow-slate-900/20 mb-4 overflow-hidden">
            ...
            <p className="text-xs text-slate-500 font-medium mb-6">Platform Administrator</p>

            <div className="w-full grid grid-cols-3 gap-2 border-t border-slate-100 pt-6">
              <div className="text-center">
                <div className="text-lg font-bold text-slate-800">{Array.isArray(usersList) ? usersList.length : '-'}</div>
              ...
              <div className="text-center">
                <div className="text-lg font-bold text-slate-800">{stats.mentors ?? '-'}</div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-slate-700">{stats.dbLoad ? `${stats.dbLoad}%` : '24%'}</span>
              </div>
            </div>
          </div>
        </div>
