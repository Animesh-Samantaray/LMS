...
const StudentDashboard = () => {
  ...
  return (
    <DashboardLayout sidebarItems={sidebarItems} roleTitle="STUDENT">
      ...
      <div className="bg-blue-600 rounded-2xl p-6 md:p-8 text-white flex flex-col md:flex-row items-start md:items-center justify-between shadow-lg shadow-blue-500/20">
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-bold text-slate-800">My Learning</h3>
            ...
            {courses.length > 0 ? (
              <div className="space-y-4">
                {courses.map((course, idx) => (
                  <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all">
                    <div className="w-12 h-12 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
                      <BookOpen size={24} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-slate-800 truncate">{course.title || course.name || 'Course Name'}</h4>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs font-medium text-slate-500">{course.progress || 0}%</span>
                      </div>
                    </div>
              ...
              <div className="text-center py-10 px-4">
                ...
                <h4 className="font-semibold text-slate-700 mb-1">No courses found</h4>
                <p className="text-sm text-slate-500">You are not enrolled in any active courses.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col items-center text-center">
            <div className="w-24 h-24 rounded-2xl bg-gradient-to-tr from-blue-500 to-indigo-600 text-white flex items-center justify-center text-3xl font-bold shadow-lg shadow-blue-500/30 mb-4 overflow-hidden">
            ...
            <div className="w-full grid grid-cols-2 gap-4 border-t border-slate-100 pt-6">
              <div className="text-center">
                <div className="text-xl font-bold text-slate-800">{courses.length}</div>
                <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mt-1">Enrolled</div>
              </div>
              <div className="text-center">
                <div className="text-xl font-bold text-slate-800">{stats.completed || 0}</div>
